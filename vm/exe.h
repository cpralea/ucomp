...
protected:
    ...
    static uint32_t imm32u(const uint8_t& data) { return uint8_to_uint32(const_cast<uint8_t&>(data)); }
    static int16_t imm16s(const uint8_t& data) { return uint8_to_int16(const_cast<uint8_t&>(data)); }
    ...
    static int16_t& uint8_to_int16(uint8_t& val) { return reinterpret_cast<int16_t&>(val); }
...
private:
    ...
    void trace(uint8_t ri, uint8_t dst, uint8_t src, int16_t idx, uint32_t iv) const;
};
